...
## Project: Eve's Garden

An isometric 3D web game with character customization.

- **Stack**: Next.js 15, React Three Fiber v9, Drei, Zustand v5, Tailwind CSS v4, TypeScript, GSAP
- **Structure**: `src/components/three/` (R3F scene), `src/components/ui/` (overlay UI), `src/game/` (Zustand stores, constants, BFS pathfinding), `src/types/`
- **Domains**: Isometric 3D scene, voxel character system, tile grid pathfinding, collectibles, game state, design panel
- **No backend**: All data is static constants. No auth, no database, no API calls.
